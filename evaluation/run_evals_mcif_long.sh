...
SYSTEMS=( 'phi4multimodal' 'desta2-8b' 'voxtral-small-24b' 'qwen2audio-7b' 'canary-v2' 'owsm4.0-ctc')
DIRECTION_PAIRS=('en-de' 'en-it' 'en-zh')

# Define constant base paths.
EVAL_MODE="ref_free_and_ref_based"
SAVING_BASE_DIR="./output_evals/mcif-long"
...
for system in "${SYSTEMS[@]}"; do
    ...
    for pair in "${DIRECTION_PAIRS[@]}"; do
        ...
        MANIFEST_LONG="${BASE_PATH}/manifests/mcif-long/${pair}.jsonl"

        OUTPUT_JSONL="${BASE_PATH}/outputs/${system}/mcif-long/${pair}.jsonl"
        OUTPUT_SEGMENTED_JSONL="${BASE_PATH}/outputs/${system}/mcif-long/${pair}_segmented.jsonl"

        # Create the target directory.
        mkdir -p "$SAVING_FOLDER"

        # Run segmentation
        python segment_long_benchmarks.py --manifest-long-path "$MANIFEST_LONG" --manifest-short-path "$MANIFEST_SHORT" --model-output-path "$OUTPUT_JSONL" --output-segmented-file "$OUTPUT_SEGMENTED_JSONL" --tokenizer-path "$NLLB_TOKENIZER"
